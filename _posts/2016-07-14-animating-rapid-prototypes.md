...
When a building is constructed, there is a blueprint or even a scaled down model which builders use as a guide that they work towards. When writing a paper, one of the most basic parts of the process is to create an outline that gives you a rough idea of what you are working towards. Why should programming be any different? The short answer here is that it isn't different. In programming we call this process prototyping. There are multiple ways to prototype applications starting with with static image mockups to programatic simulations. The best prototypes are the ones that mimic the final product without requiring the backend that powers the final product. More importantly, you want to create prototypes that will take only a small fraction of the development time. Using animations we can give the impression of logical behavior to images. This creates an illusion of functionality.   
